  ...
  try {
    ...
    if (userAccount) {
      ...
      try {
        ...
        if (errorInternal instanceof InteractionRequiredAuthError || errorInternal instanceof Error) {
          try {
            const response = await msalInstance.acquireTokenPopup(accessTokenRequest);
            console.log('Token acquired via acquireTokenPopup (fallback)');
            return response.accessToken;
          } catch (popupError) {
            console.log('acquireTokenPopup failed:', popupError);
            // Fail through to try SSO
          }
        }
      }
    }
