...
export async function acquireToken(
    ...
    // Use a short timeout so the browser's user-gesture window (~5s in Chrome)
    // is still valid for the loginPopup fallback if ssoSilent fails.
    try {
        const response = await cachedMsalInstance.ssoSilent({
            ...loginRequest,
            timeout: 2000,
        })
        return response.accessToken
